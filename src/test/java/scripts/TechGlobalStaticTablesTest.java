...
Verify the entire table
    ...
    @Test(priority = 3, description = "Validate" )
    public void validateCountryColum(){

        String[] columData = {"USA", "China", "USA", "USA", "S. Korea"};
        List<WebElement> tableColum = TableHandler.getTableColum(driver, 4);

        IntStream.range(0, columData.length).forEach(i -> {
            System.out.println(tableColum.get(i).getText());
            Assert.assertEquals(tableColum.get(i).getText(), columData[i]);
        });
    }
